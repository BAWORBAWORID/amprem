...
(function () {
    ...
    /* Cek berkala anti-devtools — butuh DUA sinyal sebelum memblokir:
     * 1) ukuran jendela anomali (outer-inner), DAN
     * 2) timing `debugger` > 500ms (hanya terjadi saat DevTools benar-benar
     *    terpasang; pada frame lambat/zoom browser/headless debugger tidak
     *    menahan eksekusi).
     * Ambang ganda + 3 deteksi beruntun mencegah "Akses Ditolak" palsu yang
     * sebelumnya mengunci user asli (mis. zoom >100% atau selisih ukuran
     * jendela di perangkat/mobile emulation). */
    var devToolsStrikes = 0;
    setInterval(function () {
        var sizeAnomaly = isDevToolsOpenBySize();
        var t0 = new Date();
        debugger; // jika devtools terbuka, baris ini berhenti → delay besar
        var timingAnomaly = (new Date() - t0) > 500;
        if (sizeAnomaly && timingAnomaly) {
            devToolsStrikes += 1;
            if (devToolsStrikes >= 3) showAccessDenied();
        } else {
            devToolsStrikes = 0;
        }
    }, 1000);
