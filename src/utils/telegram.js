...
import { DATA_DIR, PORT, newId, nowISO } from './store.js';
...
const CODE_VERSION = 'v2-inline-keyboard-2';
...
function getMenuMainText(bot, msg) {
    ...
    const roleBadge = isOwner ? 'Owner Bot' : 'Reseller / User';
    const now = new Date();
    ...
    return `Halo <b>${escHtml(name)}</b> 👋\n` +
           `<code>${dateStr} pukul ${timeStr} WIB</code>\n\n` +
           `<b>User Info</b>\n` +
           ` └ <b>ID:</b> <code>${uid}</code>\n` +
           ` └ <b>Status:</b> <code>${roleBadge}</code>\n` +
           ` └ <b>Owner ID:</b> <code>${escHtml(bot.ownerId)}</code>\n\n` +
           `<b>Bot Stats</b>\n` +
           ` └ <b>Nama Bot:</b> <code>${escHtml(bot.name)}</code>\n` +
           ` └ <b>Status System:</b> 🟢 Online 24/7\n\n` +
           `Silakan pilih menu di bawah ini untuk melihat daftar perintah:`;
}

function getMenuMainKeyboard(bot, uid) {
    const isOwner = String(uid) === String(bot.ownerId);
    const buttons = [
        [{ text: '🚀 Fitur Activator AM', callback_data: 'menu_activator' }]
    ];
    if (isOwner) {
        buttons.push([{ text: '📦 Bulk Auto Generator', callback_data: 'menu_bulk' }]);
    }
    buttons.push([{ text: 'ℹ️ Informasi Chat & ID', callback_data: 'menu_info' }]);
    return { inline_keyboard: buttons };
}

function getMenuActivatorText() {
    return `🚀 <b>FITUR ACTIVATOR ALIGHT MOTION</b>\n` +
           `━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
           `📌 <b>Daftar Perintah Aktivasi:</b>\n` +
           `• <code>/create &lt;email&gt;</code>\n` +
           `  └ Kirim tautan verifikasi magic link ke email\n\n` +
           `• <code>/verify &lt;email&gt; &lt;link&gt;</code>\n` +
           `  └ Verifikasi magic link & aktifkan status Premium\n\n` +
           `• <code>/verif &lt;email&gt; &lt;link&gt;</code>\n` +
           `  └ Alias singkat untuk /verify\n\n` +
           `💡 <i>Ketik perintah di atas di kolom chat untuk memproses akun target.</i>`;
}

function getMenuBulkText() {
    return `📦 <b>BULK AUTO GENERATOR</b>\n` +
           `━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
           `📌 <b>Daftar Perintah Massal (Khusus Owner):</b>\n` +
           `• <code>/bulk &lt;jumlah&gt; [domain]</code>\n` +
           `  └ Auto generator akun massal (maksimal ${BULK_MAX} akun)\n\n` +
           `💡 <i>Contoh: <code>/bulk 5 gmail.com</code></i>`;
}

function getMenuInfoText(bot, msg) {
    const chatId = msg?.chat?.id || '-';
    const fromId = msg?.from?.id || '-';
    return `ℹ️ <b>INFORMASI CHAT & ID</b>\n` +
           `━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
           `• <b>ID Chat / Grup Ini:</b> <code>${chatId}</code>\n` +
           `• <b>ID Telegram Anda:</b> <code>${fromId}</code>\n` +
           `• <b>Owner Bot ID:</b> <code>${escHtml(bot.ownerId)}</code>\n` +
           `• <b>Bot Username:</b> @${escHtml(bot.me?.username || 'bot')}`;
}
...
/* Rate limit ringan per chat untuk /create & /verify (anti-abuse).
   Bot dipakai reseller; batasi 20 permintaan/menit & 300/hari per chat. */
...
async function handleCreate(bot, chatId, args) {
    if (!checkRateLimit(chatId)) {
        return sendText(bot.token, chatId, '⏳ <b>TERLALU BANYAK PERMINTAAN</b>\n━━━━━━━━━━━━━━━━━━━━━━━━━━━\nMohon tunggu beberapa saat sebelum mencoba lagi.');
    }
    const email = String(args || '').trim().toLowerCase();
    if (!email || email.indexOf('@') === -1) {
        return sendText(bot.token, chatId, '⚠️ <b>FORMAT SALAH</b>\n━━━━━━━━━━━━━━━━━━━━━━━━━━━\nGunakan: <code>/create email@domain.com</code>');
    }
    await sendText(bot.token, chatId, '⏳ <b>MEMPROSES TAUTAN VERIFIKASI...</b>\n━━━━━━━━━━━━━━━━━━━━━━━━━━━\n📧 <b>Email Target:</b> <code>' + escHtml(email) + '</code>');
    const res = await callLocalAPI(bot, 'send-link', { email: email });
    const ok = res && res.success;
    let text = ok
        ? '✅ <b>TAUTAN VERIFIKASI TERKIRIM!</b>\n━━━━━━━━━━━━━━━━━━━━━━━━━━━\n'
        : '❌ <b>GAGAL MENGIRIM TAUTAN</b>\n━━━━━━━━━━━━━━━━━━━━━━━━━━━\n';
    text += '📧 <b>Email:</b> <code>' + escHtml(email) + '</code>\n';
    text += '💬 <b>Respon:</b> <code>' + escHtml(apiResultText(res)) + '</code>';
    await sendText(bot.token, chatId, text);
}

async function handleVerify(bot, chatId, args) {
    if (!checkRateLimit(chatId)) {
        return sendText(bot.token, chatId, '⏳ <b>TERLALU BANYAK PERMINTAAN</b>\n━━━━━━━━━━━━━━━━━━━━━━━━━━━\nMohon tunggu beberapa saat sebelum mencoba lagi.');
    }
    const parts = String(args || '').trim().split(/\s+/);
    const email = (parts.shift() || '').trim().toLowerCase();
    const link = parts.join(' ').trim();
    if (!email || !link) {
        return sendText(bot.token, chatId, '⚠️ <b>FORMAT SALAH</b>\n━━━━━━━━━━━━━━━━━━━━━━━━━━━\nGunakan: <code>/verify email@domain.com https://link-verifikasi...</code>');
    }
    await sendText(bot.token, chatId, '⏳ <b>MEMVERIFIKASI AKUN...</b>\n━━━━━━━━━━━━━━━━━━━━━━━━━━━\n📧 <b>Email Target:</b> <code>' + escHtml(email) + '</code>');
    const res = await callLocalAPI(bot, 'activate', { email: email, magicLink: link });
    const ok = res && res.success;
    let text = ok
...
async function handleBulk(bot, chatId, fromId, args) {
    if (String(fromId) !== String(bot.ownerId)) {
        return sendText(bot.token, chatId, '⛔ <b>AKSES DITOLAK</b>\n━━━━━━━━━━━━━━━━━━━━━━━━━━━\nPerintah <code>/bulk</code> khusus untuk Owner ID bot ini.');
    }
    const parts = String(args || '').trim().split(/\s+/);
    const count = parseInt(parts[0], 10);
    const domain = (parts[1] || '').trim() || 'random';
    if (!count || count < 1 || count > BULK_MAX) {
        return sendText(bot.token, chatId, '⚠️ <b>FORMAT SALAH</b>\n━━━━━━━━━━━━━━━━━━━━━━━━━━━\nGunakan: <code>/bulk ' + BULK_MAX + '</code> (Maksimal ' + BULK_MAX + ' akun)');
    }
    ...
    const res = await callLocalAPI(bot, 'bulk', { count: count, domain: domain, tgBotId: bot.id, tgChatId: chatId });
    const ok = res && res.success;
    let text = ok
        ? '✅ <b>BATCH GENERATOR DIMULAI!</b>\n━━━━━━━━━━━━━━━━━━━━━━━━━━━\n'
        : '❌ <b>GAGAL MEMULAI BATCH</b>\n━━━━━━━━━━━━━━━━━━━━━━━━━━━\n';
    text += '🔢 <b>Jumlah:</b> <b>' + count + '</b>\n';
    text += '🌐 <b>Domain:</b> <code>' + escHtml(domain) + '</code>\n';
    text += '💬 <b>Respon:</b> <code>' + escHtml(apiResultText(res)) + '</code>';
    await sendText(bot.token, chatId, text);
}

async function handleCallbackQuery(bot, cq) {
    ...
    if (data === 'menu_main') {
        const text = getMenuMainText(bot, cq);
        const kb = getMenuMainKeyboard(bot, uid);
        await editMessageText(bot.token, chatId, messageId, text, kb);
    } else if (data === 'menu_activator') {
        const text = getMenuActivatorText();
        const kb = getBackKeyboard();
        await editMessageText(bot.token, chatId, messageId, text, kb);
    } else if (data === 'menu_bulk') {
        const text = getMenuBulkText();
        const kb = getBackKeyboard();
        await editMessageText(bot.token, chatId, messageId, text, kb);
    } else if (data === 'menu_info') {
        const text = getMenuInfoText(bot, cq);
        const kb = getBackKeyboard();
        await editMessageText(bot.token, chatId, messageId, text, kb);
    }
}
...
async function handleMessage(bot, msg) {
    ...
    const cmd = (parts.shift() || '').split('@')[0].toLowerCase(); // dukung /cmd@BotUsername
    const args = parts.join(' ').trim();

    if (cmd === '/start' || cmd === '/help') {
        const text = getMenuMainText(bot, msg);
        const kb = getMenuMainKeyboard(bot, fromId);
        await sendText(bot.token, chatId, text, 'HTML', kb);
    } else if (cmd === '/id') {
        const text = getMenuInfoText(bot, msg);
        const kb = getBackKeyboard();
        await sendText(bot.token, chatId, text, 'HTML', kb);
    } else if (cmd === '/create') {
...
async function deployBot(input) {
    ...
    if (!isOwner) {
        ...
        if (own.length - (dupOwn ? 1 : 0) >= 1) throw new Error('Role Admin hanya dapat men-deploy maksimal 1 bot. Hapus bot lama Anda terlebih dahulu.');
    }
...
function initTelegramBots() {
    ...
    // Setelah di-clear, inited di-reset agar runner di-start ulang di bawah.
    if (registry.codeVersion && registry.codeVersion !== CODE_VERSION) {
        console.log('[TELEGRAM] Deteksi versi kode berubah (' + registry.codeVersion + ' -> ' + CODE_VERSION + '), me-restart runner...');
        registry.runners.forEach((runner) => { runner.stop(); });
        registry.runners.clear();
        registry.inited = false;
    }
    registry.codeVersion = CODE_VERSION;
